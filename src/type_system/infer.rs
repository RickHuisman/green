...
// Basic integer
// type INTEGER = Type::new_operator(0, "int", &[]);
// Basic bool
// type Bool = Type::new_operator(1, "bool", &[]);
...
fn get_type(
    ...
    if let Some(value) = env.0.get(name) {
        ...
        // TODO raise ParseError("Undefined symbol {0}".format(name))
...
fn test_env() -> (Vec<Type>, Env) {
    ...
    let mut my_env = HashMap::new();
    my_env.insert("pair".to_string(), {
